package io.thothcode.tech.auth.oauth2;

import io.thothcode.tech.auth.services.AuthUserDetailsService;
import io.thothcode.tech.gluon.entities.UserEntity;
import io.thothcode.tech.gluon.oauth2.mongo.OAuth2ClientSettings;
import io.thothcode.tech.gluon.oauth2.mongo.OAuth2TokenSettings;
import io.thothcode.tech.gluon.oauth2.mongo.constants.MongoDbSignatureAlgorithm;
import io.thothcode.tech.gluon.oauth2.mongo.constants.OAuth2AuthorizationGrantType;
import io.thothcode.tech.gluon.oauth2.mongo.constants.OAuth2ClientAuthenticationMethod;
import io.thothcode.tech.gluon.oauth2.mongo.entities.MongoDbOAuth2RegisteredClient;
import io.thothcode.tech.auth.oauth2.repositories.MongoDbRegisteredClientRepository;
...
public class MongoRegisteredClientRepository implements RegisteredClientRepository {
    ...
    public RegisteredClient findByClientId(String clientId) {
        RegisteredClient registeredClient = this.repository.findByClientId(clientId)
                .map(this::to)
                .orElse(null);
        if (registeredClient == null){
            UserEntity user = (UserEntity) authUserDetailsService.loadUserByUsername(clientId);
            if (user != null){
                registeredClient = authUserDetailsService.mapToRegisteredClient(user);
                // save it
