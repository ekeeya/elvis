...
import io.thothcode.tech.auth.oauth2.repositories.AuthorizationRepository;
...
public class MongoOAuth2AuthorizationService implements OAuth2AuthorizationService {
    ...
    public MongoOAuth2AuthorizationService(AuthorizationRepository authorizationRepository, RegisteredClientRepository registeredClientRepository, ObjectMapper objectMapper) {
    ...
    private Map<String, Object> filterNonSerializableAttributes(Map<String, Object> attributes) {
        return attributes.entrySet().stream()
                .filter(entry -> {
                    try {
                        ...
                        log.warn("Skipping non-serializable attribute: {}", entry.getKey(), e);
                        return false;
                    }
                })
    ...
    private String writeMap(Map<String, Object> metadata) {
        try {
            Map<String, Object> filteredMetadata = filterNonSerializableAttributes(metadata);
            return objectMapper.writeValueAsString(filteredMetadata);
        } catch (Exception ex) {
            log.error(ex.getMessage(), ex);
            throw new IllegalArgumentException(ex.getMessage(), ex);
        }
